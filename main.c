...
int SeleccionarUnidad(stUnidad EquipoATK[])
{

    int idElejida=-1, flag=0, aux=-1,color=0 ;
    char exit=0,redo = 0;
    ...
    do
    {
        do
        {
            idElejida=SeleccionSwitch(EquipoATK[0].equipo);
            //printf("Id enemigo %d ",idEnemigo);
            flag=VerificaVivo(idElejida,EquipoATK);
            if(flag!=1)
            {
            ...
            {
                aux = idUnidad(EquipoATK, 6, idElejida);

                color=EquipoATK[aux].equipo;
                if(color==1)
                {
                    dibujaUnidad(EquipoATK[aux].posx,EquipoATK[aux].posy,EquipoATK[aux].tipo,3);
                }
                else
                {
                    dibujaUnidad(EquipoATK[aux].posx,EquipoATK[aux].posy,EquipoATK[aux].tipo,4);
                }


                barraEstado("    Unidad seleccionada   ", "Flechas para moverse");
            }
        }
        while(flag!=1);
...
int SeleccionarEnemigo(stUnidad EquipoATK[],stUnidad EquipoDEF[], int idactivo )
{
    ...
    int idEnemigo=-1, flag=0,equipo=0, aux=-1, color=-1;
    char exit=0, exit2=-1;

    equipo=EquipoDEF[0].equipo;

    if(EquipoATK[0].equipo == 1)
    {
        color =2;
    }
    else
    ...
    do
    {
        ...
        aux = idUnidad(EquipoDEF, 6, idEnemigo);
        dibujaUnidad(EquipoDEF[aux].posx,EquipoDEF[aux].posy,EquipoDEF[aux].tipo,3);
    ...
    if (flag==1)
    {
        dibujaUnidad(EquipoDEF[aux].posx,EquipoDEF[aux].posy,EquipoDEF[aux].tipo,color);
...
void turnoUnidad(int tablero[filas][columnas], stUnidad EquipoATK[],stUnidad EquipoDEF[], int idActivo)
{
    ...
    if (idEnemigo!=-1)
    {
        barraEstado("    Seleccione un enemigo      ","   Y presione enter para atacar");
...
//se puede hacer un if check que si un equipo tiene menos unidades se copie la ultima o algo asi.. o forzarlo en la carga y listo
    ...
    int i=0, vivo=-1, flag=0,unidad=0;

    while(condicionVivos(equipo1)==1 && condicionVivos(equipo2)==1 && flag==0)
    {

        dibujaDinamico(equipo1,equipo2,tablero);
        barraEstado("    Seleccione una unidad      ","   Y presione enter para mover");
        unidad = SeleccionarUnidad(equipo1);


        if(VerificaVivo(i,equipo1)==1)
        {
            turnoUnidad(tablero,equipo1,equipo2,unidad);//dibuja dinamico dentro de turnounidad
            dibujaDinamico(equipo1,equipo2,tablero);
        ...
        else if (VerificaVivo(i,equipo1)==-1)
        {
            ...
            {
                unidad = SeleccionarUnidad(equipo1);
                turnoUnidad(tablero,equipo1,equipo2,unidad);
                dibujaDinamico(equipo1,equipo2,tablero);
        ...
        if (flag==0)
        {
            ...
            {
                unidad = SeleccionarUnidad(equipo2);
                turnoUnidad(tablero,equipo2,equipo1,unidad);
                dibujaDinamico(equipo1,equipo2,tablero);
